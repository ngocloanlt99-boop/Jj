/**
 * Pure Web Audio API 8-bit Retro Synthesizer
...
export function playFlapSound(enabled: boolean = true) {
  if (!enabled) return;
  try {
...
export function playPointSound(enabled: boolean = true) {
  if (!enabled) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
...
export function playHitSound(enabled: boolean = true) {
  if (!enabled) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
...
export function playDieSound(enabled: boolean = true) {
  if (!enabled) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
